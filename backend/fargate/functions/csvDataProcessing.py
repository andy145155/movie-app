...
def rename_columns_and_reset_index(df: pd.DataFrame) -> pd.DataFrame:
    df.rename(columns={"movie_id": "movieId"}, inplace=True, errors="raise")
...
def upload_data(similarity_df: pd.DataFrame) -> None:
    upload_to_s3(
        S3_PROCESSED_FILE_PATH["SIMILARITY"],
        MOVE_CSV_PROCESSED_BUCKET_NAME,
        similarity_df,
    )
    upload_to_dynamoDB(similarity_df, MOVIES_SIMILARITY_DYNAMO_DB_NAME)
...
def get_similarity_df(
    new_df: pd.DataFrame, overview: pd.Series, similarity: np.ndarray
) -> pd.DataFrame:
    similarity_series = pd.Series(similarity.tolist())
    similarity_df = pd.DataFrame().assign(
        movie_id=new_df["movie_id"],
        title=new_df["title"],
        overview=overview,
...
def get_recommended_movie_id(obj, similarity_df: pd.DataFrame) -> List[int]:
    result = []
    recommended = sorted(list(enumerate(obj)), reverse=True, key=lambda x: x[1])[1:8]

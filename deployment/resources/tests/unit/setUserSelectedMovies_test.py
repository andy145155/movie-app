...
@mock_aws
class TestSetUserSelectedMovies(TestCase):
    """
    Test class for the application setUserSelectedMovies Lambda function
    """
    ...
    def setUp(self) -> None:
        ...
        self.mocked_movies_dynamodb_class.table.put_item(
            Item={"movieId": 3, "index": 3}
        )

    def test_extract_and_validate_data(self) -> None:
        """
        Test extract_and_validate_data function
        """
        from functions.setUserSelectedMovies import extract_and_validate_data

        input = {
            "email": "test@gmail.com",
            "selectedMovies": [
                {"movieId": "1", "similarity": ["1", "2"]},
                {"movieId": "2", "similarity": ["1", "2"]},
            ],
        }

        expected_return_value = "test@gmail.com", [
            {"movieId": "1", "similarity": ["1", "2"]},
            {"movieId": "2", "similarity": ["1", "2"]},
        ]

        result = extract_and_validate_data(input)
    ...
    def test_lambda_handler_valid_event(
        ...
        expected_return_value = {
            ...
            "body": '{"email": "test@gmail.com", "recommendedMovies": [{"movieId": "1", "index": "1"}, {"movieId": "2", "index": "2"}], "selectedMovies": [{"movieId": "1", "index": "1"}, {"movieId": "3", "index": "3"}]}',
        }

        from functions.setUserSelectedMovies import handler

        test_event = self.load_sample_event_from_file("setUserSelectedMovies")
